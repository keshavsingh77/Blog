...
const HomePage: React.FC = () => {
  ...
  return (
    <div className="bg-white dark:bg-gray-950 min-h-screen transition-colors duration-500 pb-20">
      ...
      {featured && (
        <section className="relative w-full h-[70vh] md:h-[90vh] bg-gray-950 overflow-hidden">
          <img src={featured.imageUrl} alt={featured.title} className="w-full h-full object-cover opacity-50 transition-transform duration-[30s] scale-110 hover:scale-100" />
          <div className="absolute inset-0 bg-gradient-to-t from-gray-950 via-gray-950/60 to-transparent"></div>
          
          <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-16 lg:p-24">
            <div className="max-w-7xl mx-auto w-full">
              <span className="inline-block bg-blue-600 text-white text-[10px] font-black uppercase tracking-[0.4em] px-5 py-2 rounded-full mb-8 shadow-2xl">
                Trending / {featured.category}
              </span>
              <h1 className="text-4xl md:text-7xl lg:text-9xl font-black text-white mb-10 leading-[0.9] max-w-5xl tracking-tighter drop-shadow-2xl italic">
                {featured.title}
              </h1>
              <Link to={`/post/${featured.id}`} className="group relative inline-flex items-center bg-white text-gray-950 px-12 py-6 rounded-3xl font-black text-sm uppercase transition-all shadow-2xl hover:bg-blue-600 hover:text-white overflow-hidden">
                <span className="relative z-10 flex items-center">Discover More <i className="fas fa-arrow-right ml-4 group-hover:translate-x-2 transition-transform"></i></span>
              </Link>
            </div>
          </div>
        </section>
        ...
        <div className="max-w-7xl mx-auto px-4 flex items-center gap-4 overflow-x-auto no-scrollbar">
          {tabs.map(tab => (
            <button
              ...
              className={`px-8 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap border-2 ${
                activeTab === tab 
